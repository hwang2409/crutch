...
class EDAG:
    ...
    def _node_to_string(self, nid: str) -> str:
        ...
        if data.type == "OP":
            ...
            child_ids = (
                ...
                else list(self.g.predecessors(nid))
            )
            ...
            def wrap(child: str, parent_op: str, is_right: bool = False) -> str:
                ...
                need = (prec.get(cop, 6) < prec.get(parent_op, 6)) or (
                    is_right
                    and parent_op == "^"
                    and prec.get(cop, 6) == prec.get(parent_op, 6)
                )
            ...
            if data.op in ("+", "-", "*", "/", "^"):
                ...
                if k >= 2:
                    ...
                    if data.op == "*":
                        ...
                        if to_remove:
                            parts = [
                                p for idx, p in enumerate(parts) if idx not in to_remove
                            ]
    ...
    def _add_const(self, v: Any) -> str:
        n = self._nid()
        self.g.add_node(
            n, data=Node("CONST", str(v), value=(v if isinstance(v, Rational) else v))
        )
        return n
    ...
    def _copy_subdag(self, nid: str, out: "EDAG", memo: Dict[str, str]) -> str:
        ...
        if data.type == "CONST":
            n = out._nid()
            out.g.add_node(n, data=Node("CONST", data.symbol, value=data.value))
            memo[nid] = n
            return n
        if data.type == "VAR":
            n = out._nid()
            out.g.add_node(n, data=Node("VAR", data.symbol))
            memo[nid] = n
            return n
        # OP
        ...
        out.g.add_node(
            n,
            data=Node(
                "OP", data.op, op=data.op, is_unary=data.is_unary, children=child_copies
            ),
        )
        for c in child_copies:
            out.g.add_edge(c, n)
        memo[nid] = n
        return n
        ...
        def d(nid: str) -> str:
            ...
            if data.type == "OP":
                ...
                if data.op in (
                    ...
                    if data.op == "sin":
                        fprime = out._add_op(
                            "cos", [out._add_expr_str(self._node_to_string(u))]
                        )
                    elif data.op == "cos":
                        fprime = out._add_op(
                            "-",
                            [
                                out._add_op(
                                    "sin", [out._add_expr_str(self._node_to_string(u))]
                                )
                            ],
                            is_unary=True,
                        )
                        ...
                        fprime = out._add_op(
                            "exp", [out._add_expr_str(self._node_to_string(u))]
                        )
                        ...
                        fprime = out._add_op(
                            ...
                            [
                                out._add_const(one),
                                out._add_expr_str(self._node_to_string(u)),
                            ],
                        )
                    elif data.op == "sqrt":
                        fprime = out._add_op(
                            ...
                            [
                                ...
                                out._add_op(
                                    "*",
                                    [
                                        out._add_const(Rational(2, 1)),
                                        out._add_op(
                                            "sqrt",
                                            [
                                                out._add_expr_str(
                                                    self._node_to_string(u)
                                                )
                                            ],
                                        ),
                                    ],
                        ...
                        fprime = out._add_op(
                            "/", [out._add_const(one), out._add_op("sqrt", [one_minus])]
                        )
                    elif data.op == "acos":
                        # -1/sqrt(1-u^2)
                        one_minus = out._add_op(
                ...
                if data.op in ("+", "-"):
                    return out._add_op(
                        data.op, [d(data.children[0]), d(data.children[1])]
                    )
                if data.op == "*":
                    u, v = data.children
                    ...
                    return out._add_op(
                        "+",
                        [
                            out._add_op("*", [d(u), v_txt]),
                            out._add_op("*", [u_txt, d(v)]),
                        ],
                    )
                if data.op == "/":
                    u, v = data.children
                    num = out._add_op(
                        "-",
                        [
                            out._add_op("*", [d(u), out._copy_subdag(v, out, {})]),
                            out._add_op("*", [out._copy_subdag(u, out, {}), d(v)]),
                        ],
                    )
                    den = out._add_op(
                        "^", [out._copy_subdag(v, out, {}), out._add_const(two)]
                    )
                ...
                if data.op == "^":
                    ...
                    def is_zero_n(nid: str) -> bool:
                        ...
                        return (
                            nd.type == "CONST"
                            and isinstance(nd.value, Rational)
                            and nd.value.is_zero()
                        )
                    ...
                    if vd.type == "CONST":
                        ...
                        if isinstance(nval, Rational) and nval.is_int():
                            ...
                            u_pow = out._add_op(
                                ...
                                [
                                    out._add_expr_str(self._node_to_string(u)),
                                    out._add_const(Rational(nval.to_int() - 1, 1)),
                                ],
                            )
                            return out._add_op(
                                "*", [ncoef, out._add_op("*", [u_pow, du])]
                            )
                    ...
                    term1 = out._add_op(
                        ...
                        [
                            dv,
                            out._add_op(
                                "log", [out._add_expr_str(self._node_to_string(u))]
                            ),
                        ],
                    )
                    term2 = out._add_op(
                        "*",
                        [
                            out._add_expr_str(self._node_to_string(v)),
                            out._add_op(
                                "/", [du, out._add_expr_str(self._node_to_string(u))]
                            ),
                        ],
                    )
        ...
        def I(nid: str) -> Optional[str]:
            ...
            if n.type == "OP" and n.op == "^":
                ...
                if (
                    ...
                    and b.symbol == var
                    and e.type == "CONST"
                    and isinstance(e.value, Rational)
                    and e.value.is_int()
                ):
                    nint = e.value.to_int()
                    if nint == -1:
            ...
            if (
                ...
                if ch.type == "VAR" and ch.symbol == var:
                    ...
                    if n.op == "sin":
                        return out._add_op(
                            "-", [out._add_op("cos", [x])], is_unary=True
                        )
                    if n.op == "cos":
            ...
            if n.type == "OP" and n.op == "/" and len(n.children) == 2:
                ...
                if (
                    num.type == "CONST"
                    and isinstance(num.value, Rational)
                    and num.value == one
                    and den.type == "VAR"
                    and den.symbol == var
                ):
            ...
            if n.type == "OP" and n.op == "/" and len(n.children) == 2:
                ...
                if (
                    ...
                    and den.type == "OP"
                    and den.op == "+"
                    and len(den.children) == 2
                ):
                    left = self.g.nodes[den.children[0]]["data"]
                    right = self.g.nodes[den.children[1]]["data"]
                    if (
                        left.type == "CONST"
                        and isinstance(left.value, Rational)
                        and left.value == one
                        and right.type == "OP"
                        and right.op == "^"
                    ):
                        b = self.g.nodes[right.children[0]]["data"]
                        e = self.g.nodes[right.children[1]]["data"]
                        if (
                            b.type == "VAR"
                            and b.symbol == var
                            and e.type == "CONST"
                            and isinstance(e.value, Rational)
                            and e.value == Rational(2, 1)
                        ):
                            x = out._add_var(var)
            ...
            if n.type == "OP" and n.op == "/" and len(n.children) == 2:
                ...
                if (
                    ...
                    if (
                        inner.type == "OP"
                        and inner.op == "-"
                        and len(inner.children) == 2
                    ):
                        l = self.g.nodes[inner.children[0]]["data"]
                        r = self.g.nodes[inner.children[1]]["data"]
                        if (
                            l.type == "CONST"
                            and isinstance(l.value, Rational)
                            and l.value == one
                            and r.type == "OP"
                            and r.op == "^"
                        ):
                            b = self.g.nodes[r.children[0]]["data"]
                            e = self.g.nodes[r.children[1]]["data"]
                            if (
                                b.type == "VAR"
                                and b.symbol == var
                                and e.type == "CONST"
