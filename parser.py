    ...
    while i < n:
        ...
        if c.isdigit():
            ...
            while j < n and (s[j].isdigit() or (s[j]=='.' and not has_dot)):
                has_dot = has_dot or s[j]=='.'; j += 1
            num_str = s[i:j]
            if has_dot:
                if prev and prev.kind in ('ID','NUM',')'):
                    toks.append(PolyTok('*', '*'))
                toks.append(PolyTok('NUM', num_str, Rational(int(float(num_str)*10**6), 10**6)))
...
def poly_to_rpn(toks: List[PolyTok]) -> List[PolyTok]:
    ...
    for t in toks:
        if t.kind in ('NUM','ID'):
            ...
            while op and op[-1].kind != '(' and ((t.kind in _poly_right_assoc and _poly_prec[t.kind] < _poly_prec[op[-1].kind]) or (t.kind not in _poly_right_assoc and _poly_prec[t.kind] <= _poly_prec[op[-1].kind])):
                out.append(op.pop())
            op.append(t)
        elif t.kind == '(':
            op.append(t)
...
@dataclass
class Number(Expr): value: Rational

@dataclass
class Variable(Expr): name: str

@dataclass
class UnaryOp(Expr): op: str; arg: Expr

@dataclass
class BinaryOp(Expr): op: str; left: Expr; right: Expr

@dataclass
class FunctionCall(Expr): name: str; args: List[Expr]
    ...
    while i < n:
        ...
        if c.isdigit():
            ...
            if has_dot:
                if prev and prev.kind in ('ID','NUM',')'):
                    toks.append(ExprTok('*','*'))
                toks.append(ExprTok('NUM', num_str, Rational(int(float(num_str)*10**6), 10**6)))
                i = j; prev = toks[-1]; continue
            if j < n and s[j] == '/' and (j+1 < n and s[j+1].isdigit()):
                j2 = j+1; k2 = j2
                while k2 < n and s[k2].isdigit():
...
def expr_to_rpn(toks: List[ExprTok]) -> List[ExprTok]:
    ...
    for t in toks:
        if t.kind in ('NUM','ID'):
            ...
            while op and op[-1].kind != '(' and ((t.kind in _expr_right_assoc and _expr_prec[t.kind] < _expr_prec[op[-1].kind]) or (t.kind not in _expr_right_assoc and _expr_prec[t.kind] <= _expr_prec[op[-1].kind])):
                out.append(op.pop())
            op.append(t)
        elif t.kind == '(':
            op.append(t)
...
def rpn_to_ast(rpn: List[ExprTok]):
    ...
    for t in rpn:
        if t.kind == 'NUM':
            ...
            if not stack: raise ValueError('function missing argument')
            arg = stack.pop()
...
def rpn_to_edag(rpn: List[ExprTok]) -> EDAG:
    ...
    for t in rpn:
        if t.kind == 'NUM':
            nid = add_node(Node('CONST', t.lex, value=(t.num if t.num is not None else Rational(0,1))))
            stack.append(nid)
        elif t.kind == 'ID':
            ...
            nid = add_node(node)
            dag.g.add_edge(a, nid); dag.g.add_edge(b, nid)
            stack.append(nid)
        elif t.kind == 'FUNC':
            if not stack: raise ValueError('function missing argument')
            a = stack.pop()
